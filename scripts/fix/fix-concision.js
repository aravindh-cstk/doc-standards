...
function askForRewrite(fullLine, sentence, ruleText) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    const prompt = buildPrompt(fullLine, sentence, ruleText, priorViolation);
    let stdout;
    try {
    ...
    if (ratio < 0.4 || ratio > 1.6) {
      priorViolation = 'the reply length looked wrong (too short or too long for a single-sentence fix)';
      continue;
    }
